...
type mockPM struct {
	...
	runningFn func(name string) (int, bool)
	killFn    func(name string) error
}

func (m *mockPM) Spawn(app config.App) SpawnResult        { return m.spawnFn(app) }
func (m *mockPM) IsRunning(name string) (int, bool)       { return m.runningFn(name) }
...
func TestRunStart_AllSucceed(t *testing.T) {
	pm := &mockPM{
		runningFn: func(name string) (int, bool) { return 0, false },
		spawnFn: func(app config.App) SpawnResult {
...
// TestRunStart_AlreadyRunning verifies that apps already running are skipped, not double-launched.
func TestRunStart_AlreadyRunning(t *testing.T) {
	spawnCalled := 0
	pm := &mockPM{
		runningFn: func(name string) (int, bool) {
			return 9999, true // all apps already running
		},
		spawnFn: func(app config.App) SpawnResult {
			spawnCalled++
			return SpawnResult{PID: 100}
	...
	if spawnCalled != 0 {
		t.Errorf("Spawn called %d times, want 0 — already-running apps should be skipped", spawnCalled)
	}
	if !strings.Contains(out, "[=]") {
