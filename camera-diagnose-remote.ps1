...
Write-Host "     -> the frame server holds a stale handle. Try, on THIS machine:"
Write-Host "          motorhome.exe camera"
Write-Host "        or manually:  sc stop FrameServer   (it demand-starts again)"
Write-Host " * FrameServer NOT PRESENT -> this is likely a Server SKU without the"
Write-Host "     camera frame server; the holder is the redirection stack and the"
Write-Host "     reconnect trick below is the realistic fix."
Write-Host " * Before rebooting, try disconnecting and reconnecting the RDP"
Write-Host "     session: that tears down and rebuilds the redirected device."
